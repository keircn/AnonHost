...
import { BMCSupporter } from "@/types/bmc";

async function verifyBmcTransaction(transactionId: string) {
  if (
    process.env.NODE_ENV === "development" &&
    transactionId.startsWith("BMC_TEST_")
  ) {
  ...
  const transaction = data?.data?.find(
    (supporter: BMCSupporter) => supporter.transaction_id === transactionId,
  );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("BMC verification error:", error);
    return NextResponse.json(
