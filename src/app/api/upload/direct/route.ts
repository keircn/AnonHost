...
export async function POST(request: NextRequest) {
  ...
  const body = (await request
    .json()
    .catch(() => null)) as CreateDirectUploadBody | null;
  ...
  if (typeof body.fileName !== 'string') {
    return NextResponse.json(
      { ok: false, error: 'fileName must be a string' },
      { status: 400 }
    );
  }
  ...
  if (typeof body.fileSize !== 'number') {
    return NextResponse.json(
      { ok: false, error: 'fileSize must be a number' },
      { status: 400 }
    );
  }
  ...
  if (typeof body.contentType !== 'string') {
    return NextResponse.json(
      { ok: false, error: 'contentType must be a string' },
      { status: 400 }
    );
  }
