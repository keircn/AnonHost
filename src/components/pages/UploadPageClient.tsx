...
export function UploadPageClient() {
  ...
  const uploadDirectFile = async (
    ...
    const createResult =
      (await createResponse.json()) as ActionResult<CreateDirectUploadData>;
    if (!createResponse.ok || !createResult.ok) {
      throw new Error(
        createResult.ok ? 'Failed to start direct upload' : createResult.error
      );
    }
    ...
    if (!uploadResult.ok) {
      ...
      throw new Error(
        `Direct upload failed with status ${uploadResult.status}`
      );
    }
    ...
    if (!finalizeResponse.ok || !finalizeResult.ok) {
      ...
      throw new Error(
        finalizeResult.ok ? 'Upload finalization failed' : finalizeResult.error
      );
    }
              ...
              {!isAnonymous && privateMode && (
                <div className="mb-4 grid gap-3 rounded-lg border bg-card p-3 sm:grid-cols-[1fr_auto] sm:p-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="private-password" className="text-xs">
                      Password
                    </Label>
                    <Input
                      id="private-password"
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {isAnonymous && link.deletionUrl && (
                      <UrlRow
                        label="Deletion URL (save this)"
                        value={link.deletionUrl}
                      />
                    )}
                    {link.terminalUrl && (
                      <UrlRow label="Terminal" value={link.terminalUrl} />
                    )}
                    {link.curlCommand && (
                      <UrlRow label="Curl" value={link.curlCommand} />
                    )}
                  </div>
                </div>
              ))}
